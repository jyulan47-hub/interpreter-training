...
| R7 | 按固定目录分库存放 | 术语数据存「译前准备」文件夹，练习数据存「练习数据」文件夹，命名规则见 §10.1，不得混放、不得覆盖既有文件。 |
...
在 Skill 数据根目录下建立**两个固定文件夹**：

| 文件夹 | 存入内容 | 命名规则 |
|---|---|---|
| **译前准备/** | 专业术语数据（主题背景、术语表、双语表达、来源）—— 每次译前准备生成一份 | `{主题}_{日期}.md`，如 `人工智能_2026-09-18.md` |
| **练习数据/** | 一份练习数据包含：素材（用户上传或检索使用的视频链接）、用户翻译（含 ASR/修正稿）、AI 评分、修正与提升建议 | `{素材名}_{日期}.md`，如 `_10min_2026-09-18.md` |

规则：
- **译前准备数据**由调用「interpreter-terminology」得到的术语结果整理成 `译前准备/{主题}_{日期}.md`。
- **练习数据**（口译/视译每次一道）整理成 `练习数据/{素材名}_{日期}.md`，正文含：素材链接、方向/难度、用户翻译（可含 `original_asr` + `corrected_transcript`）、`score`+分项、修正、提升建议、参考译文。
- `日期`取当次创建日（`YYYY-MM-DD`）。
- **实行覆盖保护**：同名文件已存在时，不直接覆盖，改为追加序号（`名_日期.md`、`名_日期_2.md`…）或先询问用户（R7）。
- 术语数据进「译前准备」、练习数据进「练习数据」，**不得混放**。
- 宿主不支持目录/文件持久化 → 如实说明，并用导出的 `.md` 组织同样的两套文件供用户手动保存。
